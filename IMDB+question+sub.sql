...
-- total number of rows in director_mapping
SELECT COUNT(*) AS director_mapping
FROM director_mapping;

-- total number of rows in genre
SELECT COUNT(*) AS genre
FROM genre;

-- total number of rows in movie
SELECT COUNT(*) AS names
FROM names; 


-- total number of rows in ratings
SELECT COUNT(*) AS ratings
FROM ratings;


-- total number of rows in role_mapping
...
-- THERE are 20 null values in country_nulls, 3724 null values in world_wide_income, 
-- 194 in language and 528 null values in production_company.
...
-- total number of movies released in each month 
...
-- Total number of movies produced in India and USA are 1059
...
-- DRAMA category has the highest number of movies produced overall among all the other categories.
...
-- There are 3289 movies present which is made upon only one genre.
...
SELECT production_company,
       ...
       RANK() OVER (ORDER BY avg_rating) AS prod_company_rank
FROM 
	movie AS m
    LEFT JOIN
    ratings AS r
...
-- There are total of 361 movies released between 1 April 2018 and 1 April 2019, given a median rating of 8.
...
-- The top five actresses in Hindi movies released in India based on their average ratings are Taapsee Pannu, 
-- Kriti Sanon, Divya Dutta, Shraddha Kapoor, Kriti Kharbanda. 
